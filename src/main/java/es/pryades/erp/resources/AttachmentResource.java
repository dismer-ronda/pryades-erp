...
public class AttachmentResource extends ServerResource 
{
			...
			if ( Authorization.isValidRequest( token, ts+timeout, ts, password, timeout ) ) 
			{
				rep = new OutputRepresentation(MediaType.APPLICATION_PDF) 
				{
					...
					public void write( OutputStream arg0 ) throws IOException
					{
						try
						{
					    	QuotationAttachment query = new QuotationAttachment();
					    	query.setId( Long.parseLong( id ) );
					    	
							QuotationAttachment quotation = (QuotationAttachment)IOCManager._QuotationsAttachmentsManager.getRow( ctx, query );
